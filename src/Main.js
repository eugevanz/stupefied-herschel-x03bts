...
export function Main({user}) {
  ...
  const client = createClient(REACT_APP_SUPABASE_URL, REACT_APP_SUPABASE_SERVICE_ROLE);
  ...
  return (<div className='uk-background-muted'>
    <button className='uk-button uk-button-link uk-width-1-4@m uk-margin-right uk-margin-left uk-margin-top uk-margin-large-bottom uk-inline'>
      <div className='uk-grid-small uk-flex-middle' data-uk-grid>
        ...
        <div className='uk-width-expand uk-text-left'>
          <h3 className='uk-card-title uk-text-small uk-text-bold uk-margin-remove-bottom'>{user?.user_metadata?.full_name ?? 'NoName'}</h3>
      ...
      <div data-uk-dropdown="mode: click">
        <ul className='uk-list uk-list-large uk-list-divider'>
          ...
          <li>
            <button className='uk-button uk-button-small uk-button-danger uk-width-1-1' onClick={ () => netlifyIdentity.logout() }>Sign out</button>
          </li>
        </ul>
      </div>
    </button>
      ...
      <div data-uk-grid='masonry: true; parallax: 48'>
        ...
        <div className='uk-width-1-4@m'>
          ...
          <div className='uk-card uk-card-secondary uk-card-small uk-card-body uk-border-rounded'>
            ...
            <ul data-uk-accordion='collapsible: false'>
              { project_groups?.length ? project_groups.map(item => <li key={ item.id }>

                <a className='uk-accordion-title uk-text-bold uk-text-default' href='#list-item'>
                  { item.name }
                </a>

                { projects?.length ? <div className='uk-accordion-content'>
                  <ul className='uk-list uk-list-collapse  uk-list-square'>
                    { ListOfProjects(item.id) }
                  </ul>
                </div> : null }

              </li>) : <li><i>There are no project groups to show</i></li> }
            </ul>
          </div>
        </div>
        ...
        <div className='uk-width-expand@m uk-margin'>
          <ul id='component-tab-left' className='uk-switcher'>
            <li><Projects client={client}></Projects></li>
