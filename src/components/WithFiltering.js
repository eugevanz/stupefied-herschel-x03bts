...
export function WithFiltering({ setSelected, table, title, client }) {
      ...
      <li key={item.id} className="uk-margin">
        ...
        {item.status ? (
          <div className="uk-flex uk-margin-small">
            <select
              className="uk-select uk-form-small uk-border-rounded uk-margin-right"
              onChange={onChange}
  ...
  return (
    <div className="uk-card uk-card-small uk-card-body uk-card-default uk-border-rounded uk-margin-right">
      <div className="uk-text-bold uk-text-small uk-text-muted uk-margin-large">
        {title}
      </div>
      <ul className="uk-list">
        {table.length > 0 ? (
          table.map((item) => <ListItem key={item.id} item={item}></ListItem>)
        ) : (
          <li>
            <i>Nothing to show</i>
          </li>
